...
<nav class="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 shadow-lg z-50 safe-area-pb">
    <div class="max-w-screen-xl mx-auto px-1 sm:px-4">
        <div class="flex justify-around items-center h-16 sm:h-20">
            <g:each in="${navItems}" var="item">
                ...
                <a href="${item.href}"
                   class="flex flex-col items-center justify-center px-1 py-2 sm:px-4 sm:py-3 rounded-lg transition-colors flex-1 max-w-[90px] sm:max-w-none ${activeClass}">

                    <!-- Icon -->
                    <div class="mb-1">
                        <div class="w-6 h-6 sm:w-10 sm:h-10">
                            ${raw(item.icon.replace('w-6 h-6', 'w-6 h-6 sm:w-10 sm:h-10'))}
                        </div>
                    </div>

                    <!-- Label -->
                    <span class="text-xs sm:text-sm font-semibold truncate w-full text-center">${item.label}</span>

                    <!-- Active indicator -->
                    <g:if test="${isActive}">
                        <div class="w-1.5 h-1.5 sm:w-2 sm:h-2 bg-blue-600 rounded-full mt-0.5 sm:mt-1"></div>
                    </g:if>
                </a>
            </g:each>
        </div>
...
<style>
    ...
    @media (max-width: 640px) {
        body.has-bottom-nav {
            padding-bottom: 80px; /* 64px nav height + 16px extra padding on mobile */
        }
    }
</style>
